...
const AIMediaBuyer: React.FC<AIMediaBuyerProps> = ({ onClose }) => {
    ...
    try {
      ...
      const mockStrategy: CampaignStrategy = {
        ...
        optimizationTips: [
          ...
          "Set up daily budget caps of 20-30% of monthly budget divided by 30",
  ...
  const renderStep = () => {
    switch (currentStep) {
              ...
              <div className="grid grid-cols-2 gap-2">
                {platformOptions.map((platform) => (
                  <Button
                    ...
                    className={`justify-start ${formData.platforms.includes(platform.value) ? "bg-metamaster-primary text-white" : ""}`}
                    onClick={() => handleMultiSelect('platforms', platform.value)}
                  >
      ...
      case 3:
        return (
          <div className="space-y-4">
            <h2 className="text-lg font-medium">Audience Information</h2>
  ...
  return (
    <div className="py-2">
      ...
      {/* Navigation buttons */}
      <div className={`flex ${currentStep === 4 ? 'justify-end' : 'justify-between'} pt-4 border-t border-gray-200`}>
        {currentStep < 4 && (
          <>
            {currentStep > 1 && (
              <Button 
