...
// Retail & E-commerce
{
  ...
  value: 'specialty_retail',
  label: 'Specialty Retail'
}, {
  value: 'general_ecommerce',
...
const AIMediaBuyer: React.FC<AIMediaBuyerProps> = ({
  onClose
}) => {
  const {
    toast
  ...
  const generateCampaignStrategy = async () => {
    ...
    try {
      ...
      const mockStrategy: CampaignStrategy = {
        ...
        creativeRecommendations: ["Use lifestyle imagery showing your product in real-world situations", "Create video content demonstrating key product benefits", "Include user testimonials and reviews for social proof", "Highlight your unique value proposition in the first 3 seconds", "Test both emotion-based and logical/feature-based messaging"],
        optimizationTips: ["Start with automatic placements but monitor performance to refine later", `Set up daily budget caps of $${Math.round(formData.budgetRange[0] / 30)} - $${Math.round(formData.budgetRange[1] / 30)}`, "Use CBO (Campaign Budget Optimization) for platforms that support it", "Implement a minimum 2-week testing period before major adjustments", "Monitor frequency caps to avoid ad fatigue (keep under 2.5 for cold traffic)"]
      };
  ...
  const renderStep = () => {
    if (campaignCreated && createdCampaignData) {
      return <CampaignSuccessScreen campaignData={createdCampaignData} onClose={onClose} />;
    }
    switch (currentStep) {
      case 1:
        // Campaign & Business Information - UPDATED with new structure
        return <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input placeholder="E.g., Summer Sale 2025" value={formData.campaignName} onChange={e => handleInputChange('campaignName', e.target.value)} />
            ...
            <div className="space-y-2">
              ...
              <Select value={formData.objective} onValueChange={value => handleInputChange('objective', value)}>
                ...
                <SelectContent>
                  {objectiveOptions.map(option => <SelectItem key={option.value} value={option.value}>
                      {option.label}
                    </SelectItem>)}
                </SelectContent>
              </Select>
            ...
            <div className="space-y-2">
              ...
              <Input placeholder="Enter your business name" value={formData.businessName} onChange={e => handleInputChange('businessName', e.target.value)} />
            ...
            <div className="space-y-2">
              ...
              <Select value={formData.businessType} onValueChange={value => handleInputChange('businessType', value)}>
                ...
                <SelectContent className="max-h-72 overflow-y-auto">
                  {businessTypes.map(option => <SelectItem key={option.value} value={option.value}>
                      {option.label}
                    </SelectItem>)}
                </SelectContent>
              </Select>
            ...
            <div className="space-y-2">
              ...
              <Input placeholder="E.g., Fitness, Real Estate, Technology" value={formData.industryVertical} onChange={e => handleInputChange('industryVertical', e.target.value)} />
              <p className="text-xs text-gray-500">The industry your business operates in</p>
            </div>
            
            <div className="space-y-2">
              <label className="text-sm font-medium">Business Description</label>
              <Textarea placeholder="Describe your business and what you offer" value={formData.businessDescription} onChange={e => handleInputChange('businessDescription', e.target.value)} className="min-h-[100px]" />
            ...
            <div className="space-y-2">
              ...
              <Input placeholder="E.g., https://www.example.com" value={formData.businessWebsite} onChange={e => handleInputChange('businessWebsite', e.target.value)} />
              <p className="text-xs text-gray-500">Your main business website address</p>
            </div>
          </div>;
      case 2:
        // Target Audience - UPDATED with new structure
        return <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Textarea placeholder="Describe your target audience in detail" value={formData.targetAudience} onChange={e => handleInputChange('targetAudience', e.target.value)} className="min-h-[120px]" />
            ...
            <div className="space-y-2">
              ...
              <Input placeholder="E.g., United States, New York City, Europe" value={formData.targetLocation} onChange={e => handleInputChange('targetLocation', e.target.value)} />
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-3">
                <Input type="number" min="13" max="65" placeholder="Min Age" value={formData.ageRangeMin} onChange={e => handleInputChange('ageRangeMin', e.target.value)} className="w-24" />
                <span>to</span>
                <Input type="number" min="13" max="65+" placeholder="Max Age" value={formData.ageRangeMax} onChange={e => handleInputChange('ageRangeMax', e.target.value)} className="w-24" />
              </div>
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                {languageOptions.map(lang => <Button key={lang.value} type="button" variant={formData.languages.includes(lang.value) ? "default" : "outline"} className={formData.languages.includes(lang.value) ? "bg-metamaster-primary text-white" : ""} onClick={() => handleMultiSelect('languages', lang.value)}>
                    {formData.languages.includes(lang.value) ? <Check className="mr-2 h-4 w-4" /> : <div className="w-4 mr-2" />}
                    {lang.label}
                  </Button>)}
              </div>
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                {incomeOptions.map(inc => <Button key={inc.value} type="button" variant={formData.income.includes(inc.value) ? "default" : "outline"} className={formData.income.includes(inc.value) ? "bg-metamaster-primary text-white" : ""} onClick={() => handleMultiSelect('income', inc.value)}>
                    {formData.income.includes(inc.value) ? <Check className="mr-2 h-4 w-4" /> : <div className="w-4 mr-2" />}
                    {inc.label}
                  </Button>)}
              </div>
      ...
      case 3:
        ...
        return <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Textarea placeholder="What are your ideal customers interested in?" value={formData.customerInterests} onChange={e => handleInputChange('customerInterests', e.target.value)} className="min-h-[100px]" />
              <p className="text-xs text-gray-500">
            ...
            <div className="space-y-2">
              ...
              <Textarea placeholder="What online behaviors do your ideal customers exhibit?" value={formData.customerBehaviors} onChange={e => handleInputChange('customerBehaviors', e.target.value)} className="min-h-[100px]" />
              <p className="text-xs text-gray-500">
            ...
            <div className="space-y-2">
              ...
              <Textarea placeholder="What problems or challenges does your ideal customer face?" value={formData.customerPainPoints} onChange={e => handleInputChange('customerPainPoints', e.target.value)} className="min-h-[100px]" />
              <p className="text-xs text-gray-500">
            ...
            <div className="flex items-center space-x-2 pt-2">
              <Switch id="lookalikes-switch" checked={formData.lookalikes} onCheckedChange={checked => handleInputChange('lookalikes', checked)} />
      ...
      case 4:
        // Ad Copy - UPDATED to reduce duplication
        return <div className="space-y-4">
            <h2 className="text-lg font-medium">Ad Copy</h2>
            
            {!skipAdCopy && <div className="space-y-4">
                <div className="p-4 bg-blue-50 border border-blue-100 rounded-lg mb-4">
                  <p className="text-sm text-blue-700">
                ...
                <div className="pt-4">
                  <AdCopyGenerator embedded={true} productDetails={formData.businessWebsite || formData.businessDescription} offerDescription={formData.mainOfferDescription} />
                </div>
              </div>}
            
            {skipAdCopy && <div className="text-center py-8 text-gray-500">
      ...
      case 5:
        ...
        return <div className="space-y-6">
            <AdCreativeForm onChange={handleAdCreativeChange} initialData={formData.adCreative} />
          </div>;
      case 6:
        ...
        return <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="pt-6 pb-2">
                <Slider value={formData.budgetRange} min={500} max={20000} step={100} onValueChange={value => handleInputChange('budgetRange', value)} className="mb-6" />
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                <Input type="number" value={formData.timeframeValue} onChange={e => handleInputChange('timeframeValue', parseInt(e.target.value) || 0)} className="w-20" min={1} />
                <Select value={formData.timeframeUnit} onValueChange={value => handleInputChange('timeframeUnit', value)}>
                  <SelectTrigger className="w-32">
                    <SelectValue placeholder="Select unit" />
                  </SelectTrigger>
                  <SelectContent>
                    {timeframeUnitOptions.map(option => <SelectItem key={option.value} value={option.value}>
                        {option.label}
                      </SelectItem>)}
                  </SelectContent>
                </Select>
            ...
            <div className="pt-6">
              <Button onClick={generateCampaignStrategy} disabled={isGenerating} className="w-full">
                {isGenerating ? <>
      ...
      case 7:
        ...
        return generatedStrategy ? <div className="space-y-6">
            ...
            <Tabs defaultValue="overview" className="w-full">
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <div className="flex gap-2 flex-wrap">
                          {generatedStrategy.platforms.map(platform => <div key={platform} className="px-3 py-1 bg-metamaster-primary/10 text-metamaster-primary rounded-full text-sm">
                      ...
                      <div>
                        ...
                        <div className="space-y-2">
                          {generatedStrategy.budgetAllocation.map(allocation => <div key={allocation.platform} className="flex justify-between items-center">
                              <span className="text-sm capitalize">{allocation.platform}:</span>
                              <div className="flex items-center">
                                <div className="w-32 h-2 bg-gray-200 rounded-full mr-2">
                                  <div className="h-full bg-metamaster-primary rounded-full" style={{
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-6">
                      {generatedStrategy.audienceTargeting.map(segment => <div key={segment.segment} className="pb-4 border-b border-gray-100 last:border-0 last:pb-0">
                ...
                <Card>
                  ...
                  <CardContent>
                    <ul className="space-y-2">
                      {generatedStrategy.creativeRecommendations.map((tip, index) => <li key={index} className="flex items-start">
              ...
              <TabsContent value="optimization" className="space-y-4 pt-4">
                <Card>
                  ...
                  <CardContent>
                    <ul className="space-y-2">
                      {generatedStrategy.optimizationTips.map((tip, index) => <li key={index} className="flex items-start">
            ...
            <div className="pt-4 border-t border-gray-200 mt-6">
              ...
              <FacebookConnection onConnected={handleFacebookConnection} isConnected={fbConnected} accountData={fbAccountData} />
              
              {fbConnected && <div className="mt-6">
                  <Button onClick={handleSubmitCampaign} disabled={isSubmittingCampaign} className="w-full">
  ...
  return <div className="py-2">
      ...
      {currentStep < 7 && <div className="flex mb-8 ai-media-buyer-steps">
          {[1, 2, 3, 4, 5, 6].map((step, index) => {
        const status = getStepStatus(step);
        return <div key={step} className="flex-1 flex flex-col items-center">
                <div className="flex items-center w-full">
                  <div className={`step-circle ${status}`}>
                    {status === "completed" ? <Check size={16} /> : step}
                  </div>
                  {step < 6 && <div className={`step-line ${index < currentStep - 1 ? "completed" : ""}`} />}
                </div>
      ...
      {/* Navigation buttons */}
      {!campaignCreated && currentStep < 7 && <div className="flex justify-between pt-4 border-t border-gray-200">
          {currentStep > 1 && <Button variant="outline" onClick={prevStep} disabled={isGenerating}>
              Back
            </Button>}
          
          {currentStep === 4 && <div className="flex items-center mr-4">
              <Switch id="skip-ad-copy" checked={skipAdCopy} onCheckedChange={setSkipAdCopy} />
      ...
      {(campaignCreated || currentStep === 7) && !fbConnected && <div className="flex justify-end pt-4 border-t border-gray-200">
